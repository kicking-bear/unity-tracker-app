...
export default function StaffCodeRow({
  tournamentId, code, onChange,
}: { tournamentId: string; code: string; onChange: (c: string) => void }) {
  ...
  async function save(newCode?: string) {
    setBusy(true); setErr('')
    try {
      const r = await api.rotateCode(tournamentId, newCode)
      setDraft(r.code); onChange(r.code)
    } catch (e) { setErr((e as Error).message); setDraft(code) }
    setBusy(false)
  }
    ...
    <div className="space-y-2" onClick={e => e.stopPropagation()}>
      ...
      {!dirty && !err && (
        <p className="text-xs text-muted-foreground">
          Anyone with this code can enter scores for this tournament only.
        </p>
      )}
    </div>
  )
