...
import { matchNumbers, sideOf, stageById, tally, teamById, winnerOf } from '@/lib/tournament'
      ...
      <Card>
        <CardContent className="space-y-4">
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            <span className="font-mono text-xs font-medium text-foreground">#{nums[match.id]}</span>
        ...
        <Card>
          <CardContent className="space-y-3">
            ...
            <div className="flex items-end gap-3">
              ...
              {[A, B].map((s, k) => (
                <div key={k} className="w-24 space-y-1.5">
            ...
            {Array.from({ length: match.best_of }, (_, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <Input
                  ...
                  aria-label={`Set ${i + 1}, ${A.team?.name}`}
                  className="h-12 w-24 border-t-2 text-center font-mono text-xl tabular-nums"
                  style={{ borderTopColor: A.team?.color ?? undefined }}
                  value={val(i, 'a')}
                  onChange={e => setScores(s => ({ ...s, [`a${i}`]: e.target.value }))} />
                <Input
                  type="number" inputMode="numeric" min={0}
                  aria-label={`Set ${i + 1}, ${B.team?.name}`}
                  className="h-12 w-24 border-t-2 text-center font-mono text-xl tabular-nums"
                  style={{ borderTopColor: B.team?.color ?? undefined }}
                  value={val(i, 'b')}
                  onChange={e => setScores(s => ({ ...s, [`b${i}`]: e.target.value }))} />
              </div>
            ))}
